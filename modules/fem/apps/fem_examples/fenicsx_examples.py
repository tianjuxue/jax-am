...
def get_dog_bone_file_path(file_type, index):
    ...
    xdmf_root = f"modules/fem/apps/fem_examples/data/xdmf" 
...
def get_cylinder_file_path():
    return f"modules/fem/data/msh/cylinder.xdmf"
...
def plasticity(disps, path):
    ...
    cells = meshio_mesh.get_cells_type(cell_type)
    out_mesh = meshio.Mesh(points=meshio_mesh.points, cells={cell_type: cells})
    xdmf_file = f"modules/fem/data/msh/cylinder.xdmf"
    out_mesh.write(xdmf_file)
    mesh_xdmf_file = io.XDMFFile(MPI.COMM_WORLD, xdmf_file, 'r')
    msh = mesh_xdmf_file.read_mesh(name="Grid")
    E = 70e3
...
def performance_test():
    ...
    np.savetxt(f"modules/fem/apps/fem_examples/data/txt/fenicsx_fem_time.txt", solve_time, fmt='%.3f')
...
def generate_fem_examples():
    ...
    np.save(f'modules/fem/apps/fem_examples/data/numpy/linear_elasticity/fenicsx/disps.npy', linear_elasticity_disps)
    np.save(f'modules/fem/apps/fem_examples/data/numpy/linear_elasticity/fenicsx/forces.npy', tractions)
    ...
    for disp in hyperelasticity_disps:
        ...
        tractions.append(traction)
    tractions = np.array(tractions)
    np.save(f'modules/fem/apps/fem_examples/data/numpy/hyperelasticity/fenicsx/disps.npy', hyperelasticity_disps)
    np.save(f'modules/fem/apps/fem_examples/data/numpy/hyperelasticity/fenicsx/forces.npy', tractions)
