...
class Field:
    ...
    """
    def __init__(self):
        self.data_path = os.path.join(args['root_path'], f'data')
        self.neper_folder = f"{self.data_path}/neper/{args['case']}"
        self.pf_numpy_info_folder = f"{self.data_path}/numpy/{args['case']}/pf/info"
        self.pf_numpy_sols_folder = f"{self.data_path}/numpy/{args['case']}/pf/sols"
        self.pf_vtk_mesh_folder = f"{self.data_path}/vtk/{args['case']}/pf/mesh"
        self.pf_vtk_sols_folder = f"{self.data_path}/vtk/{args['case']}/pf/sols"
        os.makedirs(self.pf_numpy_info_folder, exist_ok=True)
        os.makedirs(self.pf_numpy_sols_folder, exist_ok=True)
        os.makedirs(self.pf_vtk_mesh_folder, exist_ok=True)
        os.makedirs(self.pf_vtk_sols_folder, exist_ok=True)
        ...
        unique_oris_rgb, unique_grain_directions = self.get_unique_ori_colors()
        ...
        # print(args['domain_x'] / points[1, 0])
        # print(args['domain_y'] / points[Nx + 1, 1])
        # print(f"Nx = {Nx}, Ny = {Ny}, {points[(Nx + 1)*(Ny + 1), 2]}")
        # exit()
    ...
    def write_info(self):
        ...
        '''
        onp.save(f"{self.pf_numpy_info_folder}/edges.npy", self.edges)
        onp.save(f"{self.pf_numpy_info_folder}/vols.npy", self.volumes)
        onp.save(f"{self.pf_numpy_info_folder}/centroids.npy", self.centroids)
    ...
    def write_vtu_files(self):
        ...
        '''
        filepath = f"{self.neper_folder}/domain.msh"
        fd_mesh = meshio.read(filepath)
        fd_mesh.write(f"{self.pf_vtk_mesh_folder}/fd_mesh.vtu")
        poly_mesh = self.obj_to_vtu()
        poly_mesh.write(f"{self.pf_vtk_mesh_folder}/poly_mesh.vtu")
    ...
    def obj_to_vtu(self):
        ...
        '''
        filepath=f"{self.neper_folder}/domain.obj"
    ...
    def convert_to_3D_images(self):
        step = 0
        filepath = f"{self.pf_vtk_sols_folder}/u{step:03d}.vtu"     
    ...
    def get_unique_ori_colors(self):
        ...
        onp.save(f"{self.data_path}/numpy/quat.npy", ori2.data)
        dx = onp.array([1., 0., 0.])
        dy = onp.array([0., 1., 0.])
        dz = onp.array([0., 0., 1.])
        ...
        if save_ipf:
            ...
            # plt.savefig(f'{self.data_path}/pdf/ipf_x.pdf', bbox_inches='tight')
            ori2.scatter("ipf", c=rgb_y, direction=ipfkey_y.direction)
            # plt.savefig(f'{self.data_path}/pdf/ipf_y.pdf', bbox_inches='tight')
            ori2.scatter("ipf", c=rgb_z, direction=ipfkey_z.direction)
            # plt.savefig(f'{self.data_path}/pdf/ipf_z.pdf', bbox_inches='tight')
...
def ipf_logo():
    ...
    plt.savefig(f"{os.path.join(args['root_path'], f'data')}/pdf/ipf_legend.pdf", bbox_inches='tight')
...
def walltime(func):
    def wrapper(*list_args, **keyword_args):
        ...
        with open(f"{os.path.join(args['root_path'], f'data')}/txt/walltime_{platform}_{args['case']}.txt", 'w') as f:
