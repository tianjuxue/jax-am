...
def get_T_fn(polycrystal, pf_args):
    def get_T_laser(t):
        ...
        vel_avg = 0.6*pf_args['domain_x'] / pf_args['t_OFF']

        Q_max = 35
        Q_min = 15
        Q = t/pf_args['t_OFF']*(Q_min - Q_max) + Q_max

        vel_max = vel_avg*1.5
        vel_min = vel_avg*0.5
        vel = t/pf_args['t_OFF']*(vel_max - vel_min) + vel_min

        X = centroids[:, 0] - x0 - (vel + vel_min)/2. * t
