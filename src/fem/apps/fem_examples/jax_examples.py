...
import time
import os
from src.fem.jax_fem import Mesh, LinearElasticity, HyperElasticity, Plasticity
...
def linear_elasticity_dogbone(disp, index):
    abaqus_root = f"src/fem/apps/fem_examples/data/abaqus" 
...
def linear_elasticity_cylinder(disps):
    ...
    np.save(f"src/fem/apps/fem_examples/data/numpy/linear_elasticity/jax_fem/forces.npy", tractions)
...
def hyperelasticity_cylinder(disps):
    ...
    for i, disp in enumerate(disps):
        ...
        tractions.append(traction[2])
    tractions = np.array(tractions)
    np.save(f"src/fem/apps/fem_examples/data/numpy/hyperelasticity/jax_fem/forces.npy", tractions)


def plasticity():
    problem_name = "plasticity"
    plasticity_path =  f"src/fem/apps/fem_examples/data/"
...
def performance_test():
    ...
    onp.savetxt(f"src/fem/apps/fem_examples/data/txt/jax_fem_{platform}_time.txt", solve_time[::-1], fmt='%.3f')
    onp.savetxt(f"src/fem/apps/fem_examples/data/txt/jax_fem_{platform}_dof.txt", num_dofs[::-1], fmt='%.3f')


def generate_fem_examples():
    plasticity()
    linear_elasticity_disps = np.load(f'src/fem/apps/fem_examples/data/numpy/linear_elasticity/fenicsx/disps.npy')
    linear_elasticity_cylinder(linear_elasticity_disps)
    hyperelasticity_disps = np.load(f'src/fem/apps/fem_examples/data/numpy/hyperelasticity/fenicsx/disps.npy')
